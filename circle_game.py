...
class Player:
    ...
    def player_move(self, p, ANGLE_MIN, ANGLE_MAX, screen):
        if not self.steps:
            ...
            for i in range(1, num_steps + 1):
                inter_x = p[-2][0] + d[0] * (distance * i / num_steps)
                inter_y = p[-2][1] + d[1] * (distance * i / num_steps)
                self.steps.append((inter_x, inter_y))
        if self.steps:
            self.x0, self.y0 = p[-1]
            self.x, self.y = self.steps.pop(0)
    ...
    def eat(self, p, food_pos_tot, food_pos_tot_flag, traj_rect_start_coords, traj_rect_end_coords):
        ...
        for i in range(len(food_pos_tot)):
            if food_pos_tot_flag[i] == 1:
                dist = np.linalg.norm([food_pos_tot[i][0] - self.x, food_pos_tot[i][1] - self.y])
...
class Game:
    def __init__(self):
        ...
        self.WALL_START_X = [self.player_radius, self.WINDOW_WIDTH, self.WINDOW_WIDTH, self.player_radius]
        self.WALL_START_Y = [self.player_radius, self.player_radius, self.WINDOW_HEIGHT, self.WINDOW_HEIGHT]
        self.WALL_END_X = [self.WINDOW_WIDTH, self.WINDOW_WIDTH, self.player_radius, self.player_radius]
        self.WALL_END_Y = [self.player_radius, self.WINDOW_HEIGHT, self.WINDOW_HEIGHT, self.player_radius]
    ...
    def game_loop(self, player1, first_pos, food_pos_tot, food_pos_tot_flag):
        ...
        while running:
            ...
            player1.player_move(p1, self.ANGLE_MIN, self.ANGLE_MAX,self.screen)
...
def main():
    ...
    game.game_loop(player1, p1_start, food_pos_tot, food_pos_tot_flag)
    sys.exit()
